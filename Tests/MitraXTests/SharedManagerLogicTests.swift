...
final class SharedManagerLogicTests: XCTestCase {
    ...
    var bar: Property<String>!

    override func setUp() {
        super.setUp()
        sut = SharedManager()
        foo = Property(value: 0, sharedManager: sut!)
        bar = Property(value: "", sharedManager: sut!)
    }
    ...
    func test_1prop_that_thread_borrows_and_this_thread_waits() {
        ...
        queue.async {
            self.sut.borrow(self.foo.rw) { foo in
                barrier.signal()
                Thread.sleep(until: Date() + secsToWait)
                foo.value = 1
            }
        }
        ...
        XCTAssertGreaterThanOrEqual(tm.executionTime, secsToWait - 0.1)
    }
